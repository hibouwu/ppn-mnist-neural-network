...
- [x] 实现 `Matrix` / `Vector` 类（支持加法、乘法、转置、广播）
- [x] 添加随机初始化与打印函数
- [x] 编写单元测试，验证矩阵运算正确性

### 2. 激活函数模块

- [x] 定义抽象类 `ActivationFunction`（接口：`forward`, `backward`）
- [x] 实现 `ReLU`, `Sigmoid`, `Tanh`
- [x] 编写测试验证输出是否正确

### 3. 线性层模块

- [x] 定义 `LinearLayer(in_dim, out_dim)` 类
- [x] 实现 `forward(x) = x @ W + b`
- [x] 添加随机初始化（高斯或均匀分布）
- [x] 测试输入输出维度是否正确

### 4. 组装基础 MLP

- [x] 定义 `MLPNetwork` 类，包含 `Linear + Activation`
- [x] 实现 `addLayer()`、`forward(input)`
- [x] 手动测试网络结构是否工作

---

## 阶段二：自动微分引擎（Autodiff Engine）

### 5. 节点与计算图基础

- [x] 实现 `Node` 类：`value`, `grad`, `parents`, `backward_fn`
- [x] 支持 `backward()` 自动拓扑排序回传梯度
- [x] 测试标量加法/乘法的反向传播结果

### 6. 向量/矩阵运算支持

- [x] 实现 `add`, `mul`（逐元素）及其反向规则
- [x] 实现 `matmul`（矩阵乘法）及反向规则：
  - `dA += grad_output @ Bᵀ`
  - `dB += Aᵀ @ grad_output`
- [x] 测试矩阵乘法梯度是否正确
...
- [x] 在自动微分框架中注册 `relu`, `sigmoid`, `tanh`
- [x] 实现各自的反向规则
- [x] 数值梯度检查验证正确性

### 8. 聚合操作

- [x] 实现 `sum`, `mean` 操作及其反向传播
- [x] 确保梯度能正确广播
...
- [x] 将 `Matrix` 替换为 `TensorNode`
- [x] 所有层运算基于自动微分节点实现

### 10. MLP 梯度计算

- [x] 调用 `loss.backward()` 自动计算梯度
- [x] 从参数节点（W, b）中提取 `.grad`
...
- [x] 实现 `MSELoss(pred, target)`
- [x] （可选）实现 `CrossEntropyLoss`
- [x] 验证损失及反向传播正确性

### 13. 优化器模块

- [x] 定义抽象类 `Optimizer`
- [x] 实现 `SGDOptimizer`（含 `step()` 与 `zero_grad()`）
- [x] 在简单函数上验证下降趋势

### 14. 初始化策略

- [x] 实现 `Xavier` 与 `Kaiming` 初始化方法
- [x] 根据激活函数自动选择初始化策略

### 15. 完整训练循环

- [x] 定义 `train_step()`：前向 → loss → backward → update
- [x] 打印每次迭代 loss 值
- [x] 确认训练过程中 loss 连续下降
...
- [x] 使用提供的读取函数加载 MNIST 数据
- [x] 归一化输入数据到 [0,1] 或 [-1,1]
- [x] 实现 DataLoader（mini-batch 生成器）
...
- [x] 在 MNIST 上训练若干 epoch
- [x] 记录 loss 与 accuracy 曲线
- [x] 在测试集上评估准确率
