...
% Logo
\logo{\includegraphics[height=0.8cm]{Images/Logo_UVSQ.jpg}}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{Images/LayersOfMLP.png}
...
{\small
\textbf{Principe du DAG}
\begin{itemize}
    \item Chaque opération = nœud
    \item Stockage valeurs intermédiaires
    \item Calcul automatique gradients
\end{itemize}
...
\begin{itemize}
    ...
    \item Mise à jour paramètres
\end{itemize}
}

\column{0.45\textwidth}
\begin{exampleblock}{\small Exemple simple}
{\scriptsize
\begin{align*}
z &= x \cdot w \\
y &= z + b \\
...
\begin{center}
\includegraphics[width=0.95\textwidth,height=0.75\textheight,keepaspectratio]{Images/conception_detaillee.png}
\end{center}
\vspace{-0.3cm}
\begin{center}
\small
...
MLPNetwork MLPNetwork::createMultiHidden(
    ...
    // Couches cachées : prev -> h + activation
...
\begin{center}
\includegraphics[width=\textwidth,height=0.52\textheight,keepaspectratio]{Images/scaling_plot.png}
\vspace{-0.15cm}
...
\column{0.49\textwidth}
\begin{center}
\includegraphics[width=\textwidth,height=0.52\textheight,keepaspectratio]{Images/scaling_speedup_plot.png}
\vspace{-0.15cm}
...
\vspace{-0.3cm}
\begin{center}
\includegraphics[width=0.88\textwidth,height=0.57\textheight,keepaspectratio]{Images/comparison_grid_plot.png}
\vspace{-0.15cm}
...
\begin{center}
\includegraphics[width=\textwidth,height=0.48\textheight,keepaspectratio]{Images/e2e_benchmark_plot.png}
\vspace{-0.15cm}
...
\vspace{-0.3cm}
\begin{center}
\includegraphics[width=0.88\textwidth,height=0.58\textheight,keepaspectratio]{Images/training_curve.png}
\end{center}
\vspace{-0.25cm}
{\scriptsize LR=0.01 | Batch=64 | Hidden=128 | ReLU | 20 époques}
...
\begin{figure}
\includegraphics[width=0.9\textwidth]{Images/combined_lr_results.png}
...
\begin{itemize}
    ...
    \item \textbf{LR ∈ [0.005, 0.01]} : \textcolor{green}{\textbf{Zone optimale}} ($\sim$98.0\%)
...
\begin{center}
\includegraphics[width=0.95\textwidth,height=0.65\textheight,keepaspectratio]{Images/combined_batch_results.png}
\vspace{-0.2cm}
...
\column{0.48\textwidth}
\begin{center}
\includegraphics[width=\textwidth,height=0.46\textheight,keepaspectratio]{Images/combined_hidden_results.png}
\end{center}
\vspace{-0.2cm}
{\tiny Taille de couche cachée}
...
\column{0.48\textwidth}
\begin{center}
\includegraphics[width=\textwidth,height=0.46\textheight,keepaspectratio]{Images/combined_activation_results.png}
\end{center}
\vspace{-0.2cm}
{\tiny Fonctions d'activation}
...
\begin{center}
\begin{tikzpicture}
\node[draw, rounded corners=10pt, fill=blue!10, inner sep=15pt, drop shadow] {
...
\begin{tabular}{rl}
\textbf{Learning Rate} & : 0.01 \\[0.2cm]
\textbf{Batch Size} & : 64 \\[0.2cm]
\textbf{Hidden Size} & : 128 \\[0.2cm]
\textbf{Activation} & : ReLU \\[0.2cm]
\textbf{Initialisation} & : He \\[0.2cm]
\end{tabular}

\vspace{0.4cm}

{\Large \textcolor{green}{\textbf{Précision validation : 98.2\%}}}
\end{minipage}
};
\end{tikzpicture}
\end{center}
...
{\small
\begin{itemize}
    \item Validation par Grid Search (3 graines/config)
    \item Équilibre performance et stabilité
    \item Temps d'entraînement optimal
\end{itemize}
}
\end{frame}
...
\begin{itemize}
    \item ✓ Implémentation complète \textbf{from scratch} en C++
    \item ✓ Moteur d'autodiff fonctionnel et élégant (DAG + lambdas)
    \item ✓ Validation ML : \textbf{$\sim$98.2\%} de précision sur MNIST
    \item ✓ Optimisation HPC : speedup \textbf{200×} sur le noyau de calcul
\end{itemize}
\end{block}
...
\begin{tikzpicture}
\draw[white, thick, rounded corners=8pt] (-5,-1.5) rectangle (5,1.5);
\node[white] at (0,0.7) {\large \textbf{Équipe du Projet}};
\node[white] at (0,0) {\normalsize Jianye Shi | Hao Qian};
\node[white] at (0,-0.5) {\normalsize Xiang Bian | Abdennour Boulmis};
\node[white] at (0,-1.1) {\footnotesize M1 CHPS -- UVSQ 2025/2026};
\end{tikzpicture}

\vspace{1.5cm}

{\footnotesize \textcolor{white}{Encadrant : Aurélien Delval}}
...
\begin{frame}[allowframebreaks]{Détails techniques : Matrix operations}
\textbf{Implémentation du produit matriciel}

\begin{lstlisting}[language=C++]
// Version naïve (ijk) - mauvaise localité cache
for (int i = 0; i < m; ++i)
    for (int j = 0; j < p; ++j)
        for (int k = 0; k < n; ++k)
            C(i,j) += A(i,k) * B(k,j);

// Version réordonnée (ikj) - meilleure localité
for (int i = 0; i < m; ++i)
    for (int k = 0; k < n; ++k)
        for (int j = 0; j < p; ++j)
            C(i,j) += A(i,k) * B(k,j);
...
\vspace{-0.1cm}
{\small \textbf{Calcul de gradient pour LinearLayer (layer.cpp)}}
...
\column{0.48\textwidth}
{\scriptsize
\textbf{Implémentation (math\_ops.cpp)}
\begin{itemize}
    \item Stockage de $X$ pendant forward
    \item Calcul des trois gradients
    \item Accumulation via \texttt{addGrad()}
\end{itemize}

\vspace{0.2cm}
\textbf{Tri topologique (node.cpp)}
\begin{itemize}
    \item DFS récursif
    \item Ordre : parents $\rightarrow$ enfants
    \item Backward : ordre inverse
    \item Gradient initial = 1.0
\end{itemize}
}
\end{columns}
...
\begin{alertblock}{\scriptsize Fonction backward() - node.cpp}
{\tiny Parcours inverse du DAG pour propager les gradients de la loss vers tous les paramètres}
\end{alertblock}
\end{frame}
