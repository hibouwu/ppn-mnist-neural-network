...
\begin{enumerate}[label=\textbf{Partie~\arabic* :}, leftmargin=*]
  \item \textbf{Contexte et État de l'art} : présentation du problème MNIST et du Perceptron Multicouche.
  \item \textbf{Implémentation} : description détaillée de l'architecture logicielle, du moteur d'autodifférentiation et des choix techniques.
  \item \textbf{Expérimentations} : double analyse portant sur les performances de calcul (HPC, profiling) et la validation de la convergence du modèle (Machine Learning).
  \item \textbf{Déroulé du projet} : présentation de l'organisation du travail et de la répartition des tâches au sein de l'équipe.
\end{enumerate}
Enfin, une \textbf{Conclusion} dresse le bilan et esquisse les perspectives, suivie des \textbf{Annexes} techniques, des références et d'un glossaire.
...
Les premières tentatives d'utilisation de réseaux de neurones pour la reconnaissance des chiffres remontent aux années 1990.
...
\begin{itemize}
  ...
  \item un mécanisme d'entraînement (optimiseur, \textit{loss}, gestion des \textit{mini-batches}),
...
Choix du langage C++ pour sa performance et son contrôle fin de la mémoire, en cohérence avec l'orientation \emph{calcul haute performance} de l'UE. Implémentation d'un moteur de calcul et d'autodifférentiation interne (Matrix, Node, MathOps) afin de comprendre explicitement la construction du graphe computationnel et le calcul des gradients \cite{goodfellow2016}, plutôt que de s'appuyer sur des bibliothèques de haut niveau.
...
\begin{itemize}
  ...
  \item Utilisation d'un tri topologique (méthode statique \texttt{topoSort}) pour parcourir le graphe lors de la phase de rétropropagation, plutôt que d'utiliser une récursion profonde.
\end{itemize}
...
Le module d'entraînement regroupe l'ensemble des composants nécessaires à l'apprentissage du modèle et à son évaluation. L'abstraction \texttt{LossFunction} permet de définir différentes fonctions de \textit{loss}, dont \texttt{MSELoss} et \texttt{CrossEntropyLoss}, utilisées respectivement pour des tests simples et pour la classification sur MNIST. La mise à jour des paramètres est assurée par la classe abstraite \texttt{Optimizer}, avec une implémentation basée sur la descente de gradient stochastique (\texttt{SGDOptimizer}).

Les données sont fournies par \texttt{MNISTDataset}, puis organisées en mini-batchs à l'aide de \texttt{DataLoader}. La classe \texttt{Trainer} coordonne l'ensemble du processus d'entraînement, en orchestrant le flux suivant : chargement des données, propagation avant dans le modèle, calcul de la \textit{loss}, rétropropagation des gradients et mise à jour des paramètres. Cette séparation claire des rôles permet de faire évoluer indépendamment le modèle, la fonction de \textit{loss} ou la stratégie d'optimisation, tout en conservant une architecture globale cohérente.

\vspace{2cm}

\subsection{Implémentation détaillée}
Cette section décrit les principaux choix d'implémentation des classes et méthodes constituant le moteur de calcul, le réseau neuronal et le pipeline d'entraînement. Après avoir présenté l'architecture globale du projet, nous détaillons ici la manière dont elle se traduit concrètement dans le code. Le diagramme UML de la Figure~\ref{fig:uml} synthétise les principales classes et relations ; l'implémentation décrite ci-dessous suit cette organisation.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Images/conception_detaillee.png}
  \caption{Diagramme de classes UML (implémentation détaillée)}
  \label{fig:uml}
\end{figure}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item l'initialisation du gradient au niveau de la fonction de \textit{loss} (explicitement, ou par défaut à 1 s'il n'est pas défini) ;
...
\paragraph{Fonctions d'activation} Les fonctions d'activation sont implémentées via l'héritage et le polymorphisme d'exécution (\emph{polymorphisme à l'exécution}). Une classe abstraite de base définit l'interface virtuelle pure \texttt{forward()}, permettant d'interchanger dynamiquement les algorithmes (\texttt{ReLU}, \texttt{Sigmoid}, \texttt{Tanh}) sans impacter le reste du réseau.
...
\subsubsection{Implémentation de la fonction de \textit{loss} et de l'optimisation}
L'apprentissage du réseau repose sur l'association d'une fonction de \textit{loss} et d'un algorithme d'optimisation, tous deux intégrés au moteur d'autodifférentiation. Les fonctions de \textit{loss} sont définies via une interface abstraite \texttt{LossFunction}, qui impose la méthode \texttt{forward(pred, target)}. Celle-ci construit un nœud scalaire représentant la \textit{loss} du mini-batch, directement inséré dans le graphe de calcul.

Deux fonctions de \textit{loss} ont été implémentées :
\begin{enumerate}[label=\arabic*.]
  \item \textbf{MSELoss} : utilisée principalement pour les tests et la validation du moteur d'autodifférentiation. Elle correspond à l'erreur quadratique moyenne entre prédictions et cibles, la \textit{loss} retournée étant la somme sur le mini-batch, normalisée par le nombre de sorties.
  \item \textbf{CrossEntropyLoss} : utilisée pour la classification MNIST. Elle opère directement sur les \textit{logits} du réseau et intègre une opération de \textit{softmax} stable numériquement. Le gradient par rapport aux \textit{logits} s'exprime simplement comme la différence entre les probabilités prédites et les labels \textit{one-hot}, ce qui garantit une rétropropagation efficace et stable.
\end{enumerate}
...
Au début de chaque époque, le \texttt{DataLoader} est réinitialisé afin de parcourir l'ensemble des mini-batchs. Pour chaque mini-batch, les matrices d'entrée et de cibles sont encapsulées dans des nœuds constants du graphe computationnel. La propagation avant est ensuite réalisée en appliquant successivement le modèle (\texttt{MLPNetwork}) aux données d'entrée, produisant les prédictions du réseau.

La fonction de \textit{loss} est alors évaluée à partir des prédictions et des cibles, générant un nœud de \textit{loss} généralement scalaire. La valeur numérique de la \textit{loss} est accumulée afin de calculer une \textit{loss} moyenne sur l'ensemble de l'époque. En parallèle, la précision (\textit{accuracy}) est estimée au niveau du \texttt{Trainer} en comparant, pour chaque échantillon du mini-batch, la classe prédite (\textit{argmax} des \textit{logits}) à la classe cible encodée en \textit{one-hot}.

Lorsque l'époque est exécutée en mode entraînement, la phase de rétropropagation est déclenchée pour chaque mini-batch. Les gradients des paramètres sont d'abord réinitialisés, puis la méthode \texttt{backward} est appelée sur le nœud de \textit{loss} afin de propager les gradients dans le graphe computationnel. Les paramètres du réseau sont enfin mis à jour à l'aide de l'optimiseur. En mode évaluation, les étapes de rétropropagation et de mise à jour des paramètres sont désactivées, ce qui permet de mesurer les performances du modèle sans modifier son état interne.

À la fin de l'époque, la \textit{loss} moyenne et la précision globale sont calculées à partir des quantités accumulées et retournées sous forme de métriques.
...
Afin de limiter la variabilité des résultats, les campagnes de mesure suivent un protocole systématique de verrouillage de fréquence et d'affinité CPU. Les huit premiers cœurs physiques sont forcés à 4.0~GHz à l'aide de \texttt{cpupower}, puis les exécutions sont liées explicitement à ce jeu de cœurs via \texttt{taskset}. Le détail des commandes de configuration (verrouillage de fréquence à 4.0~GHz, isolation des cœurs) est documenté dans le fichier \texttt{scripts/README.md} et appliqué via les scripts \texttt{benchmark\_affinity.sh} et \texttt{benchmark\_large.sh}. Un exemple de script est également donné en Annexe~\ref{app:protocol}. Ce protocole s'inspire des bonnes pratiques de notre ancien cours, adaptées ici à l'architecture AMD Ryzen. Les mesures de temps sont réalisées par le programme C++ \texttt{tests/test\_benchmark\_large.cpp}, dédié au benchmarking du noyau de calcul, qui intègre une horloge haute résolution (\texttt{std::chrono}) pour exclure les surcoûts liés au système d'exploitation. Le protocole inclut systématiquement un préchauffage (\emph{warm-up}) de 5 itérations pour stabiliser les caches (L1/L2) et le \emph{branch predictor} avant le début des mesures.

\paragraph{Maîtrise de l'aléatoire}
Au-delà du contrôle matériel, la rigueur expérimentale est assurée par le verrouillage des sources d'aléatoire logiciel. L'initialisation des poids et le mélange des données (\textit{data shuffling}) dépendent d'une graine unique fixée via la ligne de commande (\texttt{--seed}). Cela permet une reproduction bit-à-bit des courbes d'apprentissage et des métriques finales, condition indispensable pour comparer objectivement l'impact des différentes méthodes d'initialisation ou des hyperparamètres.
...
\begin{itemize}
    \item \textbf{Courbe bleue (\textit{train})} : performance sur les données d'entraînement. Elle reflète la capacité du modèle à s'ajuster aux données vues.
    \item \textbf{Courbe rouge (\textit{validation})} : performance sur les données de validation, jamais vues pendant l'entraînement.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Taux trop faible ($LR=0.001$)} : Le modèle converge mais trop lentement. Après 10 époques, la précision de validation plafonne à $\sim97.2\%$, inférieure à celle obtenue avec des taux plus élevés. Les mises à jour des poids sont trop petites pour atteindre l'optimum rapidement.
    ...
    \item \textbf{Taux trop élevé ($LR=0.025$)} : On observe une instabilité critique (divergence). Les pas de gradient sont trop grands, faisant "sauter" l'optimiseur par-dessus les minima de la \textit{loss}. La précision s'effondre à $\sim32.1\%$ avec une variance élevée ($\sigma \approx 0.06$), rendant l'entraînement inefficace.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Grands Batchs ($128, 256$)} : La performance se dégrade avec $B=128$, et le modèle diverge complètement avec $B=256$. Pour stabiliser $B=256$, il faudrait probablement ajuster le $LR$ (\textit{linear scaling rule}) ou utiliser un \textit{warm-up}, mais avec $LR=0.01$ fixe, cela échoue.
\end{itemize}
...
\begin{itemize}
    \item \textbf{ReLU (Manual Init) [$\sim98.24\%$]} : Notre initialisation manuelle $[-0.1, 0.1]$ a obtenu une performance comparable à l'initialisation de He. Cela s'explique par le fait que pour $N_{in}=784$, l'écart-type de cette distribution uniforme ($\sim0.057$) est très proche de l'optimum théorique de He ($\sim0.05$).
...
\begin{itemize}
    ...
    \item \textbf{Optimisation Mémoire} : Implémenter un \textit{memory pool} ou une arène d'allocation pour supprimer le surcoût de \texttt{malloc}/\texttt{free} identifié lors du profilage.
    \item \textbf{Parallélisme de Modèles} : Entraîner plusieurs instances du réseau en parallèle (par exemple pour l'exploration d'hyperparamètres), en complément du parallélisme de données déjà exploité par les \textit{mini-batchs}.
    ...
    \item \textbf{Parallélisme Distribué (MPI)} : Étendre l'entraînement à plusieurs nœuds de calcul en utilisant MPI pour paralléliser le traitement des \textit{mini-batchs} (\textit{data parallelism}).
\end{itemize}
