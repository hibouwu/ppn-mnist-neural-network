...
目前，这一路径已经能够稳定完成 \texttt{Tiny-ImageNet} 的数据装载与训练执行，但仍有明显限制。输入管线尚未覆盖异步预取、解码重叠、缓存和更完整的数据增强；模型侧则仍需继续检查 \texttt{im2col/col2im}、卷积 backward 以及卷积与 GEMM 之间的数据重排成本。后续工作因此仍需并行推进输入系统与模型训练，并计划在集群上继续开展 ResNet 和 IST 训练，以提升 Tiny-ImageNet 的测试准确率。
