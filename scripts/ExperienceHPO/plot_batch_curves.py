...
# Configuration
DATA_DIR = "output/ExperienceHPO/batch"
...
def load_data():
    ...
    for f in all_files:
        try:
            ...
            # Expected format: batch_{BATCH}_seed{Seed}.csv
...
def plot_single(df, metric_col, title, ylabel, ax, legend=False):
    sns.lineplot(data=df, x='epoch', y=metric_col, hue='batch', palette='tab10', 
...
def main():
    ...
    # Ensure Batch is treated as categorical/numeric for plotting
    # It is int, so Tab10 will treat it as distinct levels which is good
    
    print("Generating combined plot...")
    
    fig = plt.figure(figsize=(16, 12))
    # Using 3 rows, 2 columns layout with hspace=0.4 to avoid overlap
    ...
    # Move legend
    # ax_train_loss.legend(title='Batch Size') # Automatically done by legend=True
    
    fig.suptitle(f"Batch Size Experiment Results ({df['epoch'].max()} Epochs)", fontsize=16)
    
    out_path = os.path.join(PLOT_DIR, "combined_batch_results.png")
