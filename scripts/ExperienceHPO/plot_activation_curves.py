...
DATA_DIR = "output/ExperienceHPO/activation"
...
def plot_single(df, metric_col, title, ylabel, ax, legend=False):
    sns.lineplot(data=df, x='epoch', y=metric_col, hue='activation', palette='tab10', 
...
def main():
    ...
    fig.suptitle(f"Activation Function Experiment Results ({df['epoch'].max()} Epochs)", fontsize=16)
    
    out_path = os.path.join(PLOT_DIR, "combined_activation_results.png")
