...
# Configuration
DATA_DIR = "output/ExperienceHPO/lr"
...
def plot_single(df, metric_col, title, ylabel, ax, legend=False):
    sns.lineplot(data=df, x='epoch', y=metric_col, hue='lr', palette='tab10', 
...
def plot_broken(df, metric_col, title, ylabel, ax_high, ax_low, legend=False):
    ...
    sns.lineplot(data=df, x='epoch', y=metric_col, hue='lr', palette='tab10', 
                 marker='o', markersize=4, linewidth=1.5, alpha=0.9, ax=ax_low, legend=False) # No legend on bottom
    ...
    if not high_acc_data.empty:
        ...
        top_limit = min(1.0, y_max + margin)
        bottom_limit = max(0.5, y_min - margin)
        ax_high.set_ylim(bottom_limit, top_limit + 0.002)
    else:
    ...
    ax_high.set_ylabel("") # Manual label placement if needed, or rely on ax placement
    ax_low.set_ylabel("")
    ax_low.set_xlabel("Epoch")
    
    # Shared Y-label hack
    # ax_high.text(-0.15, 0.0, ylabel, transform=ax_high.transAxes, rotation='vertical', va='center')
...
def main():
    ...
    # Row 0: Train Loss | Test Loss
    # Row 1: Train Acc (High) | Test Acc (High)
    # Row 2: Train Acc (Low)  | Test Acc (Low)
    ...
    # Add Y labels manually for broken axes combined
    ...
    # Handle Legend
    # Only show legend on Train Loss (top left)
    # ax_train_loss.legend(title='LR') is already set by legend=True
    
    # Main Title
    fig.suptitle(f"Learning Rate Experiment Results ({df['epoch'].max()} Epochs)", fontsize=16)
    
    out_path = os.path.join(PLOT_DIR, "combined_lr_results.png")
