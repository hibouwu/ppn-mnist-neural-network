...
DATA_DIR = "output/ExperienceHPO/init"
...
def plot_single(df, metric_col, title, ylabel, ax, legend=False):
    sns.lineplot(data=df, x='epoch', y=metric_col, hue='init', palette='tab10', 
...
def plot_broken(df, metric_col, title, ylabel, ax_high, ax_low, legend=False):
    ...
    if not high_acc_data.empty:
        ...
        top_limit = min(1.0, y_max + margin) # slight headroom
...
def main():
    ...
    fig.suptitle(f"Initialization Experiment Results ({df['epoch'].max()} Epochs)", fontsize=16)
    
    out_path = os.path.join(PLOT_DIR, "combined_init_results.png")
