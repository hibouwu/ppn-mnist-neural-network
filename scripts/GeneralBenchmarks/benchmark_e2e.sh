...
echo "Implementation,Mean,StdDev" > output/GeneralBenchmarks/e2e_results.csv
...
for impl in "${implementations[@]}"; do
    ...
    # Compute Stats
    stats=$(echo -e "$timings" | python3 scripts/GeneralBenchmarks/compute_stats.py)
    # output format: "X.XXXXXX +/- Y.YYYYYY"
    mean=$(echo "$stats" | awk '{print $1}')
    std=$(echo "$stats" | awk '{print $3}')
    
    echo "$impl,$mean,$std" >> output/GeneralBenchmarks/e2e_results.csv
done

echo "E2E Benchmark Completed."
cat output/GeneralBenchmarks/e2e_results.csv
